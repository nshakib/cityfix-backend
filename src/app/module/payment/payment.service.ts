...
import { createBkashPayment, executeBkashPayment } from '../../services/bkash.service';
...
const createPayment = async (fineId: string, userId: string) => {
  ...
  if (fine.recipientId !== userId) throw new AppError(httpStatus.FORBIDDEN, 'Unauthorized');
...
const handleWebhook = async (payload: any) => {
  ...
  if (!paymentID) throw new AppError(httpStatus.BAD_REQUEST, 'Missing payment ID');
...
const getPaymentById = async (paymentId: string, userId: string, userRole: string) => {
  const payment = await prisma.payment.findUnique({
    where: { id: paymentId },
    include: { fine: { select: { reason: true, amount: true, recipientId: true } } }
  });
